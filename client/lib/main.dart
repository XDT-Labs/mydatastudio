...
/// `MainApp.llmServiceToken.valueOrNull`; worker isolates pass the token they
/// received via their spawn args.
Map<String, String> aiServerAuthHeaders(String? token) =>
...
class MainApp extends StatefulWidget {
  ...
  static final BehaviorSubject<String?> llmServiceUrl =
      BehaviorSubject<String?>();
  // Random per-spawn bearer token for the local AI server. Generated by
  // PythonManager, passed to the subprocess via env, and attached to every
  // request so only the paired client (not any other local process) can call it.
  static final BehaviorSubject<String?> llmServiceToken =
      BehaviorSubject<String?>();
