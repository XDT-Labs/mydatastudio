...
class ViewStateService {
  ...
  final BehaviorSubject<PhotoViewMode> viewMode =
      BehaviorSubject<PhotoViewMode>.seeded(PhotoViewMode.grid);
  ...
  final BehaviorSubject<File?> lightboxMedia = BehaviorSubject<File?>.seeded(
    null,
  );
  final BehaviorSubject<PhotoFilter> activeFilter =
      BehaviorSubject<PhotoFilter>.seeded(const PhotoFilter());
  final BehaviorSubject<String> activeNav = BehaviorSubject<String>.seeded(
    'all',
  );
  final BehaviorSubject<double> gridItemSize = BehaviorSubject<double>.seeded(
    160.0,
  );
