...
- **NEW — `generation_lock` is held across SSE yields** (open, 2026-07-26, raised in PR review). In `routes.py::_sse_stream` the `with get_generation_lock():` block wraps the whole `yield` loop, so the lock stays held while the generator is suspended waiting for the client to consume each chunk. A client that stays connected but stops reading (TCP zero-window) therefore blocks **all** local generation, including the non-streaming path that shares the lock. Scope: the local llama_cpp path only — the cloud stream (`_cloud_sse_stream`) takes no lock. Mitigating facts: it **self-heals on disconnect** (a closed socket raises into the generator, `GeneratorExit` unwinds the `with`), and the app's own `cancelStream()` closes the connection, so the realistic trigger is a wedged-but-open localhost client — narrow for a single-user desktop app. **Not fixed here, deliberately:** the obvious remedy (produce into a queue on a worker thread, release the lock before yielding) has an internal tension — a *bounded* queue just relocates the stall, because the producer then blocks on `put` while still holding the lock, whereas an unbounded one trades a liveness bug for a memory one. A correct fix needs a policy decision (bounded queue **plus** a `put` timeout that requests stop and abandons the generation, capping both memory and lock-hold time), and it rewrites the core streaming path — not something to land unvalidated at the tail of an unrelated PR, since this environment has no model loaded to smoke-test streaming against. Recommend a dedicated change with an explicit buffer bound + overflow policy.
