...
- **A server started without `APP_SUPPORT_DIR`/`AICHAT_MODELS_DIR` cannot see models that are on disk.** It resolves the models dir relative to itself, so `/util/model-status` answers `{"exists": false}` for a model sitting in Application Support, and both embedding isolates park on `Embedding model not downloaded yet. Sleeping...` indefinitely. Nothing errors — the app looks healthy and simply never embeds anything. `launch_client.sh` now sets both; if you start the server by hand, set them too (the `aichat server (python)` debug config in `.vscode/launch.json` is the reference).
