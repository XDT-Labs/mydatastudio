...
class EmbeddingIsolate {
  ...
  Future<void> start(String storagePath, String dbName, SendPort writerPort, RootIsolateToken token) async {
    ...
    _receivePort?.listen((data) {
      if (data is SendPort) {
        ...
        if (MainApp.llmServiceUrl.value != null) {
        ...
        if (type == 'log') {
          ...
          switch (level) {
            case 'info':
              ...
              break;
            case 'error':
              logger.e('[EmbeddingIsolate] $msg', error: data['error'], stackTrace: data['stackTrace']);
              break;
            case 'warning':
  ...
  static Future<void> _isolateEntry(Map<String, dynamic> cfg) async {
    ...
    BackgroundIsolateBinaryMessenger.ensureInitialized(cfg['token'] as RootIsolateToken);
    ...
    while (true) {
      try {
        ...
        for (final file in files) {
          try {
            ...
            if (file.path.startsWith('gdrive://')) {
              embedding = await _processGDriveFile(file, repo, serviceUrl!, logger);
            } else {
        ...
        logger.e("Error in EmbeddingIsolate loop: $e", error: e, stackTrace: stack);
  ...
  static Future<List<double>?> _processLocalFile(File file, String serviceUrl, AppLogger logger) async {
  ...
  static Future<List<double>?> _processGDriveFile(File file, DatabaseRepository repo, String serviceUrl, AppLogger logger) async {
    ...
    final nearExpiry = collection.expiration == null ||
        now.isAfter(collection.expiration!.subtract(const Duration(minutes: 5)));
    ...
    final driveApi = drive.DriveApi(GoogleAuthClient({'Authorization': 'Bearer $accessToken'}));
  ...
  static Future<List<double>?> _generateEmbedding(List<int> bytes, String serviceUrl, AppLogger logger) async {
