...
- **Notes:** **Pass 1 (2026-07-23) — interactive controls, not yet complete.** Focused on the headline gaps: named/roled the custom tap targets and closed the icon-only-button label holes.
  ...
  - **Still open (M4 stays unchecked):** (1) **Focus-visible (2.4.7)** audit and keyboard-traversal order. (2) A real macOS **VoiceOver** pass to validate announcements. (3) Confirm remaining custom tap targets meet the target-size minimum (only the aichat send/stop control was enlarged to 44×44 so far). (4) Resolve the mismatched photos AppBar tooltips when that module is built out.
