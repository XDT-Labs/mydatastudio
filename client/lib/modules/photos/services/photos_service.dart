...
class PhotosService extends RxService<PhotosServiceCommand, List<File>> {
  ...
  final BehaviorSubject<Map<String, List<File>>> photosByDate =
      BehaviorSubject<Map<String, List<File>>>();
  ...
  final BehaviorSubject<Map<String, int>> locationCounts =
      BehaviorSubject<Map<String, int>>();
  final BehaviorSubject<Map<String, int>> collectionPhotoCounts =
      BehaviorSubject<Map<String, int>>();

  Future<List<File>> refresh() async {
    final filter =
        ViewStateService.instance.activeFilter.valueOrNull ??
        const PhotoFilter();
  ...
  Future<List<File>> invoke(PhotosServiceCommand command) async {
    ...
    try {
      ...
      final withLocation = await repo.photosWithLocation(
