...
from .test_routes import _registry_row
...
class TestChatCompletionModelLoading:
    ...
    async def test_model_not_found_locally_returns_404(self):
        ...
        # The row has to exist for resolution to get as far as the disk: a
        # missing row is a different 404 (see test_routes).
        with patch('aichat.routes.get_llm_instance', return_value=None), \
             ...
             patch('aichat.routes.model_registry.lookup',
                   return_value=_registry_row(alias="some/model")), \
    ...
    async def test_model_switch_loads_new_model(self):
        ...
        new_mock_llm = Mock()
        new_mock_llm.create_chat_completion = Mock(return_value={
        ...
        with patch('aichat.routes.get_llm_instance', return_value=new_mock_llm), \
             ...
             patch('aichat.routes.get_locks') as mock_locks, \
             patch('aichat.routes.model_registry.lookup',
                   return_value=_registry_row(alias="new-model")), \
