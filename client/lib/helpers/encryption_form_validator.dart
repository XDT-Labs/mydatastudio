...
class EncryptionFormValidator extends Validator<dynamic> {
  ...
  /// must not be null.
  EncryptionFormValidator(this.controlName, this.compareControlName);
  ...
  Map<String, dynamic>? validate(AbstractControl<dynamic> control) {
    ...
    if (pubKeyControl.value.toString().isEmpty ||
        priKeyControl.value.toString().isEmpty) {
      final error = {ValidationMessage.required: 'Fields are required'};
      return error;
    }

    //parse strings from file
    try {
      RSAPublicKey publicKeyFromDisk =
          parser.parse(pubKeyControl.value) as RSAPublicKey;
      RSAPrivateKey privateKeyFromDisk =
          parser.parse(priKeyControl.value) as RSAPrivateKey;

      //do a simple encrypt/decrypt to make sure it works
      var encryptedString = helper.rsaEncrypt(
        publicKeyFromDisk,
        Uint8List.fromList(utf8.encode('Hello World')),
      );
      var decryptedString = helper.rsaDecryptAsString(
        privateKeyFromDisk,
        encryptedString,
      );
