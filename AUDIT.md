...
- **Notes:** **Pass 1 (2026-07-23) — interactive controls, not yet complete.** Focused on the headline gaps: named/roled the custom tap targets and closed the icon-only-button label holes.
  ...
  - **Still open (M4 stays unchecked):** (1) **Keyboard operability (2.1.1)** — the custom `GestureDetector` controls are labeled and now large enough, but are still not keyboard-focusable/activatable; the proper fix is converting them to real focusable `InkWell`/`IconButton` widgets (deferred — it changes ripple/focus-ring visuals and needs design sign-off). (2) **Focus-visible (2.4.7)** — depends on (1); nothing to highlight until the controls are focusable. (3) A real macOS **VoiceOver** pass to validate announcements + focus order (needs the app running under assistive tech). (4) Resolve the mismatched photos AppBar tooltips when that module is built out.
