...
void main() {
  ...
  setUp(() async {
    ...
    // The settings page is only reachable after login, i.e. with the vault
    // unlocked — and _saveProvider now refuses to write while it is locked,
    // because it would overwrite the stored ciphertext with blanks.
    await VaultManager.instance.createAndUnlock('${tempDir.path}/keys', 'pw');
  });
  ...
  group('SettingsDrawer tests', () {
    testWidgets(
      'renders Collections category and expands Providers under it when route is /settings',
      (tester) async {
    ...
    testWidgets(
      'renders AI Chat category and expands Models/Skills under it when route is /settings/aichat-models',
      (tester) async {
        final router = GoRouter(
  ...
  group('SettingsPage tests', () {
    testWidgets(
      'renders Google provider without API Key field, and others with API Key field',
      (tester) async {
    ...
    testWidgets(
      'typing in client ID triggers database update automatically after debounce',
      (tester) async {
        tester.view.physicalSize = const Size(1600, 3000);
        tester.view.devicePixelRatio = 1.0;
        addTearDown(() {
        ...
        await tester.enterText(
          clientIdTextFieldFinder,
          'my-new-google-client-id',
        );
        await tester.pump();
        ...
        await tester.runAsync(() async {
          ...
          expect(
            rows.isEmpty ||
                rows.first['client_id'] != 'my-new-google-client-id',
            isTrue,
          );
        });
    ...
    testWidgets('a locked vault does not overwrite the stored secret', (
      tester,
    ) async {
      // Why this matters: _loadProviders cannot decrypt while locked, so it
      // leaves the secret fields blank. _saveProvider writes all three columns
      // on every edit, and encrypt() passes empty strings through untouched —
      // so editing only the Client ID would replace a real, encrypted
      // client_secret with '' and the credential would be gone for good.
      tester.view.physicalSize = const Size(1600, 3000);
      tester.view.devicePixelRatio = 1.0;
      addTearDown(() {
        tester.view.resetPhysicalSize();
      ...
      await tester.runAsync(() async {
        ...
        await db.execute(
          ...
          "VALUES ('google', 'existing-id', ?, '', 'collection')",
          [CredentialCodec.encrypt('super-secret')],
        );
      });

      // Lock *after* seeding, mirroring a session where the vault was never
      // unlocked (or was locked again) before the page was opened.
      VaultManager.instance.lock();
      ...
      await tester.enterText(find.byType(TextField).first, 'edited-id');
      await tester.pump();
      await tester.pump(const Duration(milliseconds: 800));
      await tester.runAsync(() async {
        await Future.delayed(const Duration(milliseconds: 500));
      ...
      await tester.runAsync(() async {
        ...
        expect(rows.first['client_secret'], isNotEmpty);

        // And it is still the real secret, not a re-encrypted blank.
        await VaultManager.instance.unlock('${tempDir.path}/keys', 'pw');
        expect(
          CredentialCodec.decrypt(rows.first['client_secret'] as String?),
          equals('super-secret'),
        );
      });
    });
