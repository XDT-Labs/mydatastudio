...
import 'package:mydatatools/models/tables/collection.dart' as m;
...
void main() {
  ...
  group('DatabaseRepository', () {
    ...
    test('check Collections tables exists', () async {
      ...
      var tables = (databaseManager.database)?.allTables;
    ...
    test("Delete Collection", () async {
      ...
      List<m.Collection> allItems =
          await db?.select(db.collections).get() ?? [];
      expect(allItems.length, equals(1));

      await db?.delete(db.collections).delete(collection);

      List<m.Collection> afterDeleteItems =
          await db?.select(db.collections).get() ?? [];
    ...
    test("update nullable props in db", () async {
      ...
      m.Collection? c2 =
          await (db?.select(db.collections)
            ?..where((tbl) => tbl.id.equals(c.id)))?.getSingleOrNull();
      ...
      expect(
        c2?.lastScanDate?.difference(c.lastScanDate!).inSeconds,
        equals(0),
      );
